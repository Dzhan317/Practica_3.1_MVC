...
<template>
  <div>
    ...
    <input
      ...
      placeholder="Busca el producto que más te gusta de nuestro catálogo"
      @keyup.enter="busquedaProducto"
      class="search-input"
    />
    <button @click="busquedaProducto">Buscar</button>
    ...
    <div v-else class="grid">
      <div v-for="p in productos" :key="p.id" class="card">
        ...
        <h3>{{ p.nombre }}</h3>
        <p>{{ p.descripcion }}</p>
        <strong>{{ p.precio }} soles</strong><br>
    ...
    <p v-if="totalResultados > 0" style="color: green; font-weight: bold;">
